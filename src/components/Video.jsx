...
const Video = () => {
  ...
  const { src, endTime, askStartTime, loopStartTime, loopEndTime, options } =
    useMemo(() => {
      return {
        endTime: video?.endTime
          ? parseStringTimeInSeconds(video?.endTime)
          : duration,
  ...
  const videoClasses = classNames(
    'absolute top-0 left-0 transition-opacity duration-1000 h-full w-full',
    {
      'opacity-100': show,
      'opacity-0': !show,
    }
  ...
  const handleOnProgress = (e) => {
    updatePlayer({
      played: Math.ceil(e.playedSeconds),
    });
  };
